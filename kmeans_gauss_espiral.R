...
plot(all_data, xlim = xlim, ylim = ylim)

xall <- cbind(all_data$x, as.numeric(all_data$classes))

xall_c1 <- xall[which(xall[,n+1]==C2_LABEL),]
xall_c2 <- xall[which(xall[,n+1]==C1_LABEL),]
...
par(new=T)
contour(seqi, seqj, M1_c1, xlim = xlim, ylim = ylim) # mostra que é uma funcao continua, que junta as gaussianas todas

plot(xall_c2[,1], xall_c2[,2], col=kmeansret_c2$cluster, lwd = 2, xlim = xlim, ylim = ylim)
par(new=T)
contour(seqi, seqj, M1_c2, xlim = xlim, ylim = ylim) # mostra que é uma funcao continua, que junta as gaussianas todas
